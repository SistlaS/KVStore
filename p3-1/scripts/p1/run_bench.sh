...
# Create server-side log directory exists
...
run_testcases() {
    local test_num="$1"
    local OUTPUT_FILE="${OUTPUT_DIR}/testcases_${test_num}.txt"
    local EXP_START_TIME=$(date +%s)

    restart_server

    echo "Running: just p1::testcase $test_num $ADDR"
    just p1::testcase "$test_num" "$ADDR" | tee "$OUTPUT_FILE"
...
run_fuzz() {
    local n_clients="$1"
    local workload="$2"
    local OUTPUT_FILE="${OUTPUT_DIR}/fuzz_${n_clients}_${workload}.txt"
    local EXP_START_TIME=$(date +%s)

    restart_server

    echo "Running: just p1::fuzz $n_clients $workload $ADDR"
    just p1::fuzz "$n_clients" "$workload" "$ADDR" | tee "$OUTPUT_FILE"
